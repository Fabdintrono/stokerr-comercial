  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <InvoiceModal
        ...
        products={products.map((p) => ({ id: p.id, name: p.name, sku: (p as any).sku || "", unit: (p as any).unit || "", hasVariants: (p as any).hasVariants ?? false, hasBatches: (p as any).hasBatches ?? false, costPrice: Number((p as any).costPrice ?? 0) }))}
        onSave={handleSave}
      />
    </div>
  );
