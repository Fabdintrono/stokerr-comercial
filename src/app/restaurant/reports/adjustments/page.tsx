  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const url = locId ? `/api/inventory/movement?locationId=${locId}&limit=200` : "/api/inventory/movement?limit=200";
        ...
        <div className="rounded-lg border border-zinc-800 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-800">
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-zinc-500">Sin movimientos</td></tr>
              ) : filtered.map(m => {
                ...
                return (
                  <tr key={m.id} className="hover:bg-zinc-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3"><div className="text-white font-medium">{m.product.name}</div></td>
